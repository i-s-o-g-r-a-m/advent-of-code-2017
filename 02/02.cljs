...
(defn munge-input [input]
  (map
    #(map js/parseInt %1)
    (map #(s/split (s/trim %1) #"\s+") (s/split input "\n"))))

(defn even-div? [n d] (zero? (mod n d)))

(defn others-in-seq [me everyone] (filter #(not= %1 me) everyone))

(defn find-pair [me others]
  (remove nil? (map #(when (even-div? me %1) [me %1]) others)))

(defn find-pairs-in-row [row]
  (flatten (remove empty? (map #(find-pair %1 (others-in-seq %1 row)) row))))

(def result
  (reduce + (map #(apply / %1) (map find-pairs-in-row (munge-input input)))))
