PIPES = [
    ...
    for x in [x.split("<->") for x in open("input.txt", "r").read().strip().split("\n")]
]
...
class Graph:
    ...
    def find_groups(self):
        ...
        for n in self.nodes:
            if n in seen:
                continue
            group_nodes = Graph.walk(n)
            seen.extend(group_nodes)
            groups += 1
        return groups
...
def main():
    ...
    group_count = g.find_groups()
    assert group_count == 200
    print(group_count)
